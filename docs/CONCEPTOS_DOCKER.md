...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8047
EXPOSE 8047
...
| `EXPOSE 8047` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTFLASK["SERVIDOR WEB (front)<br/>contenedor front-flask<br/>hostname: front-flask · escucha en 8049"]
    ...
    FRONTFLASK -->|"http://api-facturas:8047<br/>al controlador, por el NOMBRE"| APIFACTURAS
    SQLSERVERINIT -->|"espera el healthcheck,<br/>siembra y termina"| SQLSERVER
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13347"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15447"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11447"| SQLSERVER
```
...
```yaml
  postgres:
    ...
    ports:
      - "15447:5432"                 # "puerto en su PC : puerto interno"
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8047`, `localhost:15447`); entre contenedores, nombres de
...
docker network create proyecto_construccion_de_software6_default

# 2. postgres
docker run -d --name postgres --network proyecto_construccion_de_software6_default --restart unless-stopped `
  ...
  -p 15447:5432 postgres:16-alpine

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. sqlserver
docker run -d --name sqlserver --network proyecto_construccion_de_software6_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Construccion123!" `
  -e "MSSQL_PID=Developer" `
  -v mssqldata:/var/opt/mssql `
  -p 11447:1433 mcr.microsoft.com/mssql/server:2022-latest

# 5. ESPERAR a que responda de verdad… mirándolo a ojo

# 6. sqlserver-init
docker run -d --name sqlserver-init --network proyecto_construccion_de_software6_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Construccion123!" `
  -v "${PWD}/db:/scripts:ro" mcr.microsoft.com/mssql/server:2022-latest /scripts/init_sqlserver.sh

# 7. mariadb
docker run -d --name mariadb --network proyecto_construccion_de_software6_default --restart unless-stopped `
...
docker run -d --name api-facturas --network proyecto_construccion_de_software6_default --restart unless-stopped `
  ...
  -v /app/bin `
  -v /app/obj `
  -p 8047:8047 api-facturas

# 10. Construir la imagen de front-flask y encenderla
docker build -t front-flask ./front_flask
docker run -d --name front-flask --network proyecto_construccion_de_software6_default --restart unless-stopped `
  -e "API_FACTURAS_URL=http://api-facturas:8047" `
  -e "CLAVE_SESION=Construccion123!Sesion" `
  -v "${PWD}/front_flask:/app" `
  -p 8049:8049 front-flask
...
```yaml
URL_API: http://api-facturas:8047
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8047` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8047` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8047:8047`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8047`, el front
...
```powershell
docker network inspect proyecto_construccion_de_software6_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_construccion_de_software6` produce
la red `proyecto_construccion_de_software6_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_construccion_de_software6-front-flask-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_construccion_de_software6_default has active endpoints (name:"proyecto_construccion_de_software6-front-flask-1" …)
```
